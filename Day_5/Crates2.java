...
public class Crates2 {
    public static void main(String[] args) {
        try {
            ...
            // ... this is embarassing straight up tho
            ...
            Stack<String> s9 = new Stack<String>();
                ...
                s9.push("S");
                stacks.add(s9);
            // congrats you made it to the bottom :P

            int count = 0; // how many items we are moving
            int from = 0; // from which stack
            int to = 0; // to which stack
            Stack<String> h1 = new Stack<String>(); 

            while (s.hasNext()) {
                token = s.nextLine();
                System.out.println(token);
                System.out.println(extractInt(token));
                Scanner readInt = new Scanner(extractInt(token));

                System.out.println(count = readInt.nextInt());
                System.out.println(from = readInt.nextInt());
                System.out.println(to = readInt.nextInt());
                readInt.close();

                for(int i=0; i<count; i++){
                    h1.push((stacks.get(from-1)).pop());
                }
                for(int i=0; i<count; i++){
                    (stacks.get(to-1)).push(h1.pop());
                }
            }
